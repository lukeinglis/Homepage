...
export default async function handler(
  ...
  const primarySession = getSession(req);
  if (!primarySession || !primarySession.email) {
    res.status(401).json({ error: "Unauthorized" });
    return;
  }

  const session = getWorkSession(req);
  if (!session || !session.accessToken) {
    res.json({ unread: 0, starred: 0, messages: [], connected: false });
    return;
  }
